...
import com.bacbpl.iptv.jetfit.ui.fragments.DetailsFragment
...
import com.bacbpl.iptv.jetfit.ui.fragments.DetailsVideoFragment
import com.bacbpl.iptv.jetfit.utils.DetailsTransition
...
class DetailsActivity : FragmentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Create ObjectSerializable from bundle or get from intent
        val objectSerializable = createObjectSerializableFromIntent()

        // Initialize fragments
        detailsFragment = DetailsFragment().newInstance(objectSerializable)
    ...
    private fun createObjectSerializableFromIntent(): ObjectSerializable {
        // Try to get from bundle first (new method)
        val bundle = intent.getBundleExtra(AppConstants.ITEM_BUNDLE)
        if (bundle != null) {
            ...
            if (id != -1 && type != null) {
                ...
                if (obj != null) {
                    return ObjectSerializable(obj)
                }
            }
        }

        // Fallback to old method
        return intent.getSerializableExtra(AppConstants.ITEM) as? ObjectSerializable
            ?: throw IllegalArgumentException("No item found in intent")
    }
    ...
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        ...
        if (event.action == KeyEvent.ACTION_DOWN) {
            when (event.keyCode) {
                ...
                KeyEvent.KEYCODE_DPAD_UP -> return hideDetails()
                KeyEvent.KEYCODE_DPAD_DOWN -> return showDetails()
            }
        }
